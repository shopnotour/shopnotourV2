...
class LocationController extends Controller
{
    ...
    public function searchForSelect2Flight( Request $request ){
        $search = $request->query('search');
        // $query = DB::select('bravo_airport.*', 'bravo_airport.name as title')->where("bravo_airport.status","publish");
        // if ($search) {
        //     $query->where('bravo_airport.name', 'like', '%' . $search . '%');
        ...
        $res = DB::table('bravo_airport')
        ->select('bravo_airport.*', 'bravo_airport.name as title')
        ->where("bravo_airport.status","publish")
        ->where('bravo_airport.name', 'like', '%' . $search . '%')
        ->orWhere('bravo_airport.code', 'like', '%' . $search . '%')
        ->orWhere('bravo_airport.address', 'like', '%' . $search . '%')
        ->get();
