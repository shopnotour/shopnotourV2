...
class Booking extends BaseModel
{
    ...
    // Add this relationship to your Booking model
    public function cancellationRequest()
    {
        return $this->hasOne(BookingCancellationRequest::class);
    }
    ...
    // ✅ Add Vendor Relationship (if needed)
...
//        return $is_api;
    ...
    {
        return md5(uniqid() . rand(0, 99999).time());
    }
...
//        return $limit;
        $q = Booking::query()
            ->select('id', 'code', 'status', 'total', 'paid', 'pnr_id', 'created_at')
            ->where('status', 'booked');
    ...
    public static function getBookingHistory($booking_status = false, $customer_id_or_name = false,  $pnr = false, $from = false, $to = false)  {
//        return $pnr;
        $list_booking = parent::query()->orderBy('id', 'desc');
        $list_booking->select('id', 'code', 'status','pnr_id','object_model','created_at','start_date','end_date','total','paid');
        ...
        if (!empty($pnr)) {
            $list_booking->where('pnr_id', 'like', "%$pnr%");
        }
...
//        $list_booking->whereIn('object_model', array_keys(get_bookable_services()));
    ...
    public static function getBookingHistorynew($booking_status = false, $customer_id_or_name = false, $vendor_id = false, $service = false, $from = false, $to = false, $pnr = false)
    {
        $list_booking = parent::query()->orderBy('id', 'desc');
//return $list_booking;
        if (!empty($booking_status)) {
        ...
        if (!empty($customer_id_or_name)) {
            $list_booking->where(function($q) use($customer_id_or_name) {
                $q->orWhere('customer_id', $customer_id_or_name)
                    ->orWhere('first_name', 'like', "%$customer_id_or_name%")
                    ->orWhere('last_name', 'like', "%$customer_id_or_name%");
            });
        }
        ...
        if (!empty($from) && !empty($to)) {
            ...
            $list_booking->whereDate('created_at', '>=', $from);
        } elseif (!empty($to)) {
            $list_booking->whereDate('created_at', '<=', $to);
        }
...
//    }
    ...
    public static function getTopCardsReportForVendor($user_id)
    {
        $total_money = parent::selectRaw('
            ...
            SUM(CASE WHEN `status` = "booked" THEN `total` ELSE 0 END) AS total_earning
        ')
            ->where('customer_id', $user_id)
            ->whereNotIn('status', static::$notAcceptedStatus)
            ->first();

        $total_booking = parent::where('customer_id', $user_id)
            ->whereNotIn('status', static::$notAcceptedStatus)
            ->count('id');

        $total_paid = parent::where('customer_id', $user_id)
            ->whereNotIn('status', static::$notAcceptedStatus)
            ->sum('paid');
        ...
        return [
            [
                ...
                'amount' => format_money_main($total_money->total_price ?? 0),
                'desc'   => __("Total booking amount"),
                'class'  => 'purple',
            ],
            [
                'title'  => __("Paid"),
                'amount' => format_money_main($total_paid ?? 0),
                'desc'   => __("Total paid amount"),
                'class'  => 'info',
            ],
            [
                'title'  => __("Due"),
                'amount' => format_money_main(($total_money->total_price ?? 0) - ($total_paid ?? 0)),
                'desc'   => __("Total due amount"),
                'class'  => 'pink',
            ],
            [
                'title'  => __("Bookings"),
...
//    }
    ...
    public static function getEarningChartDataForVendor($from, $to, $user_id)
    {
        ...
        $query = function($from, $to) use ($user_id, $sql_raw) {
            return parent::selectRaw(implode(",", $sql_raw))
                ->where('customer_id', $user_id)
        ...
        if (($to - $from) / DAY_IN_SECONDS > 90) {
            ...
            for ($month = 1; $month <= 12; $month++) {
                ...
                $data['datasets'][0]['data'][] = $row->total_paid ?? 0;
                $data['datasets'][1]['data'][] = ($row->total_price ?? 0) - ($row->total_paid ?? 0);
            }
        } elseif (($to - $from) <= DAY_IN_SECONDS) {
            // Report By Hours
            for ($i = strtotime(date('Y-m-d', $from)); $i <= strtotime(date('Y-m-d 23:59:59', $to)); $i += HOUR_IN_SECONDS) {
    ...
    {
        return $this->hasMany(BookingRoute::class, 'booking_id');
    }
