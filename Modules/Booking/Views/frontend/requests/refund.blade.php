...
@push('css')
    ...
    <style>
        ...
        #refundRequestsTable tbody tr:nth-child(even) td { background-color: #f8fafc; }
        #refundRequestsTable tbody tr:hover td           { background-color: #eff6ff !important; }
        #refundRequestsTable tbody tr.selected td        { background-color: #dbeafe !important; }
        #refundRequestsTable tbody tr                    { cursor: pointer; }

        #refundRequestsTable_filter input {
        ...
        #refundRequestsTable_filter input:focus {
            border-color: #3b82f6;
            box-shadow: 0 0 0 3px rgba(59,130,246,.15);
        }
        #refundRequestsTable_length select {
            border: 1px solid #d1d5db;
            border-radius: 6px;
            padding: 4px 8px;
            font-size: 0.82rem;
        }
        #refundRequestsTable_info,
        #refundRequestsTable_length label,
        #refundRequestsTable_filter label { font-size: 0.8rem; color: #6b7280; }

        #refundRequestsTable_paginate .paginate_button {
        ...
        #refundRequestsTable_paginate .paginate_button.current {
            background: #1e3a5f !important;
            color: #fff !important;
            border-color: #1e3a5f !important;
        }
        #refundRequestsTable_paginate .paginate_button:hover:not(.current):not(.disabled) {
        ...
        @media (max-width: 600px) {
            #refundRequestsTable_wrapper > div { display: flex; flex-direction: column; gap: 8px; }
            #refundRequestsTable_filter, #refundRequestsTable_length,
            #refundRequestsTable_info,   #refundRequestsTable_paginate {
                width: 100%; text-align: left !important; float: none !important;
            }
            #refundRequestsTable thead th,
            #refundRequestsTable tbody td { padding: 7px 8px; font-size: 0.76rem; }
        }
    </style>
@endpush
    ...
    <div class="px-3 py-4 sm:px-5 md:px-6">
        ...
        <div class="bg-white rounded-xl shadow-sm border border-gray-100 w-full overflow-hidden">
            <div class="p-4 sm:p-5 w-full min-w-0">
                ...
                <table id="refundRequestsTable" style="width:100%">
                    <thead>
                    <tr>
                        ...
                        <tr>
                            ...
                            <td>
                                {{ $request->passenger_id
                                    ? (is_array($request->passenger_id) ? implode(', ', $request->passenger_id) : $request->passenger_id)
                                    : 'All' }}
                            </td>
                            <td>
                                <span class="r-badge b-info">{{ ucfirst($request->refund_type) }}</span>
                            </td>
                            <td>{{ number_format($request->refund_amount, 2) }}</td>
                            <td>{{ number_format($request->refund_charges, 2) }}</td>
                            <td>{{ number_format($request->service_charge, 2) }}</td>
                            <td>
                                    <span style="font-weight:700;color:#065f46">
                                        {{ number_format($request->net_refund_amount, 2) }}
                                    </span>
                            </td>
                            ...
                            <td>
                                @if($request->status === 'waiting_user_approval')
                                    <div style="display:flex;flex-wrap:wrap;gap:4px">
                                        <a href="{{ route('user.refund.approve', $request->id) }}"
                                           ...
                                           onclick="return confirm('Approve this refund request?\nNet refund amount: {{ number_format($request->net_refund_amount, 2) }}')">
                                            <i class="fa fa-check"></i> Approve
                                        </a>
                                        <a href="{{ route('user.refund.reject', $request->id) }}"
        ...
        $(document).ready(function () {
            $('#refundRequestsTable').DataTable({
                ...
                columnDefs: [
                    ...
                    { responsivePriority: 2,  targets: 8  },                    // Status        — সবসময়
                    { responsivePriority: 3,  targets: 9  },                    // Requested At  — সবসময়
                    { responsivePriority: 4,  targets: 6  },                    // Net Amount    — সবসময়
                    { responsivePriority: 5,  targets: 10, orderable: false },  // Action        — সবসময়
                    { responsivePriority: 6,  targets: 1  },                    // PNR
                    { responsivePriority: 7,  targets: 3  },                    // Refund Type
                    { responsivePriority: 8,  targets: 4  },                    // Refund Amount
                    { responsivePriority: 9,  targets: 5  },                    // Charges
                    { responsivePriority: 10, targets: 2  },                    // Passenger
                    { responsivePriority: 11, targets: 7  },                    // Reason — আগে লুকাবে
                ],
                ...
                language: {
                    ...
                    emptyTable: '{{ __("No refund requests found.") }}',
