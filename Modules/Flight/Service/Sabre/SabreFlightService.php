...
//         \Log::info('Sabre search response', [
//             'total_groups' => count($response['groupedItineraryResponse']['itineraryGroups'] ?? []),
//         ]);
...
//     }
    ...
    public function search($searchData)
    {
        ...
        \Log::info('Sabre full result', ['result' => $response]);

        $result = $this->transformSabreResponse($response);

        // ✅ Airline filter
        if (!empty($searchData['airline_codes']) && !empty($result['flights'])) {
            $airlineCodes = is_string($searchData['airline_codes'])
                ? explode(',', $searchData['airline_codes'])
                : $searchData['airline_codes'];

            $airlineCodes = array_map('strtoupper', array_map('trim', $airlineCodes));

            $result['flights'] = array_values(
                array_filter($result['flights'], function ($flight) use ($airlineCodes) {
                    return in_array(
                        strtoupper($flight['validating_carrier'] ?? ''),
                        $airlineCodes
                    );
                })
            );
            $result['total_results'] = count($result['flights']);
        }

        // ✅ Cabin filter — Business/First যেন Economy search এ না আসে
        // if (!empty($searchData['travel_class']) && !empty($result['flights'])) {
        //     $cabinMap = [
        //         'ECONOMY'         => ['Y', 'W'],
        //         'PREMIUM_ECONOMY' => ['W'],
        //         'BUSINESS'        => ['C', 'J'],
        ...
        //     $allowedCodes = $cabinMap[strtoupper($searchData['travel_class'])] ?? null;

        //     if ($allowedCodes) {
        //         $result['flights'] = array_values(
        //             array_filter($result['flights'], function ($flight) use ($allowedCodes) {
        //                 $cabinCode = strtoupper(
        //                     $flight['legs'][0]['segments'][0]['fare_info']['cabin_code'] ?? ''
        //                 );
        //                 return in_array($cabinCode, $allowedCodes);
        //             })
        //         );
        //         $result['total_results'] = count($result['flights']);
        //     }
    ...
    private function callSabreApi(array $payload): array
    {
        ...
        if (!$response->successful()) {
            // ✅ Warning level — production এ log হবে
            Log::warning('Sabre API call failed', [
                'status' => $response->status(),
                'endpoint' => '/v5/offers/shop',
                'error' => $response->json('message') ?? $response->body(),
            ]);

            throw new \Exception('Sabre API call failed: ' . $response->status());
        }
    ...
    //                 ],
    //                 "AirTravelerAvail" => [[
    //                     "PassengerTypeQuantity" => $this->buildPassengerTypeQuantity(
    //                         $searchData['passengers'],
    //                         $searchData['special_fare'] ?? null
    //                     ),
    //                 ]],
    //             ],
    //             "TravelPreferences" => $travelPreferences,
    ...
    //     // ✅ Alternate Dates — env থেকে
    //     if (env('FLIGHT_ALTERNATE_DATES_ENABLED', false)) {
    //         $range = (int) env('FLIGHT_ALTERNATE_DATES_RANGE', 3);
    //         $range = max(1, min(3, $range)); // ✅ Sabre max 3

    //         $payload["OTA_AirLowFareSearchRQ"]["TPA_Extensions"]["AlternateDateRange"] = [
    ...
    {
        $travelPreferences = [
            "CabinPref" => $this->buildCabinPref($searchData['travel_class'] ?? 'ECONOMY'),
            "TPA_Extensions" => [
                "NumTrips" => [
                    "Number" => (int)config('sabre.search.num_trips')
                ],
    ...
    /**
     * Build CabinPref array for Sabre
     * Default: Economy — সব available fare class (Z,X,Q,K,L,V,M,H,B,Y)
     * Client selected: সেই cabin এর সব available fare class
     */
    private function buildCabinPref(string $travelClass): array
    {
        $cabin = $this->mapTravelClass($travelClass);
    ...
    private function buildPassengerTypeQuantity(array $passengers, ?string $specialFare = null): array
    {
        $passengerTypes = [];

        // ✅ Special fare code ঠিক করো
        $adultCode = $this->resolvePassengerCode($specialFare);
    ...
    /**
     * ✅ Special fare অনুযায়ী passenger code ঠিক করো
     * ENV বন্ধ থাকলে ADT return করবে
     */
    private function resolvePassengerCode(?string $specialFare): string
    {
        if (empty($specialFare)) {
            return 'ADT';
        }

        $fareMap = [
            'student' => [
                'enabled' => env('FLIGHT_STUDENT_FARE_ENABLED', false),
                'code' => 'STU',
            ],
            'group' => [
                'enabled' => env('FLIGHT_GROUP_FARE_ENABLED', false),
                'code' => 'GRP',
            ],
            'senior' => [
                'enabled' => env('FLIGHT_SENIOR_FARE_ENABLED', false),
                'code' => 'SNN',
            ],
            'seaman' => [
                'enabled' => env('FLIGHT_SEAMAN_FARE_ENABLED', false),
                'code' => 'SEA',
            ],
        ];
        ...
        // ✅ ENV বন্ধ থাকলে বা unknown fare হলে ADT
...
//         }
    ...
    ): array
    {
        ...
        $lastLeg = end($legs);
        $lastLegDesc = $legDescs[$lastLeg['ref'] - 1] ?? [];
        $lastSchedules = $lastLegDesc['schedules'] ?? [];
...
//    }
     ...
     * Get cabin name from code
     */
    // Cabin Code (Y/W/C/F) → Cabin Name
    private function getCabinName(?string $cabinCode): ?string
    {
        ...
        return $cabins[strtoupper($cabinCode)] ?? 'Economy';
    }
