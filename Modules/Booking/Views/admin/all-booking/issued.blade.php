    ...
    <div class="container-fluid px-3">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
            <h1 class="title-bar mb-0">
                <i class="fa fa-check-circle text-success"></i> {{ __('Issued Bookings') }}
                <span class="badge badge-success ml-2">{{ $rows->total() }} {{ __('bookings') }}</span>
            </h1>
            <div>
                <a href="{{ route('all-booking.issued') }}" class="btn btn-outline-secondary btn-sm">
        ...
        <div class="card shadow-sm mb-3">
            <div class="card-body py-2 px-3">
                <form method="GET" action="{{ route('all-booking.issued') }}" id="filter-form">
                        ...
                        <div class="col-auto">
                            ...
                            <div class="d-flex gap-1">
                                ...
                                @if(!empty($dateFrom) || !empty($dateTo) || !empty($searchTerm))
                                    <a href="{{ route('all-booking.issued') }}" class="btn btn-secondary btn-sm">
                    ...
                    <table id="bookings-table" class="table table-hover table-bordered table-sm mb-0" style="width:100%">
                        ...
                        @forelse($rows as $i => $booking)
                            ...
                            <tr>
                                ...
                                {{-- Status --}}
                                <td>
                                    <span class="badge badge-issued" style="font-size:11px">
                                        <i class="fa fa-check-circle"></i> issued
                                    </span>
                                    @if($hasTicket)
...
@push('css')
    ...
    <style>
        .badge-issued {
            background: linear-gradient(135deg, #1ceee3, #15d0d6);
            color: #fff;
            padding-left: 16px;
            position: relative;
        }
...
$(document).ready(function() {
    ...
    $(document).on('click', '.btn-pnr-edit', function(e) {
        ...
        $('#pnr_edit_status').val($(this).data('status') || 'issued');
