...
class ModuleProvider extends ModuleServiceProvider
{
    ...
    public static function getAdminMenu()
    {
        ...
        return [
            ...
            'bonus' => [
                'position'   => 15,
                'url'        => route('user.admin.bonus.index'),
                'title'      => __('Bonus & Points'),
                'icon'       => 'fa fa-gift',
                'permission' => 'bonus_manage',
                'group'      => 'system',
                'children'   => [
                    'bonus_manage' => [
                        'url'        => route('user.admin.bonus.index'),
                        'title'      => __('Give Bonus'),
                        'icon'       => 'fa fa-gift',
                        'permission' => 'bonus_give',
                    ],
                    'bonus_transactions' => [
                        'url'        => route('user.admin.bonus.transactions'),
                        'title'      => __('Bonus Transactions'),
                        'icon'       => 'fa fa-list',
                        'permission' => 'bonus_transactions',
                    ],
                ],
            ],
        ];
...
//            ];
        ...
        if (setting_item('bonus_enabled')) {
            ...
            $showBonus  = empty($bonusRoles) || in_array($user->role_id, $bonusRoles);

            if ($showBonus) {
                $res['bonus'] = [
                    'position' => 86,
                    'icon'     => 'fa fa-gift',
                    'url'      => route('user.bonus'),
                    'title'    => __("Bonus & Points"),
                ];
            }
        }
