            ...
            <div class="container-fluid">
                <div class="row">
                    <div class="col-md-6 copy-right">
                        {{date('Y')}} &copy; {{__('Shopno Tour by')}} <a
                            href="{{__('https://www.shopnosoftware.com')}}" target="_blank"
                        >{{__('Shopno Software')}}</a>
                    </div>
                    <div class="col-md-6">
                        <div class="text-md-right footer-links d-none d-sm-block">
                            <!-- <a href="{{__('https://www.shopnosoftware.com')}}" target="_blank">{{__('About Us')}}</a>
                            <a href="{{__('https://shopnosoftware.com/')}}" target="_blank">{{__('Contact Us')}}</a> -->
                        </div>
                    </div>
                </div>
            </div>
...
<script>
    $(document).ready(function() {
        // Auto expand Flight menu
        if (window.location.pathname.indexOf('/admin/module/flight') !== -1) {
            $('.main-menu a[href*="/admin/module/flight"]').each(function() {
                var $link = $(this);
                var $menuItem = $link.closest('.menu-item');

                // Open menu
                $menuItem.addClass('active');
                $menuItem.find('.children').show();
                $menuItem.find('.btn-toggle i').removeClass('fa-angle-left').addClass('fa-angle-down');
