<template>
    <div class="fc-wrap">
        ...
        <div class="fc-body">
                        ...
                        <div class="fc-left">
                            ...
                            <div class="fc-airline">
                                ...
                                <div class="fc-airline-info">
                                    ...
                                    <!-- ✅ Inline info strip: flight no + aircraft + cabin + seats -->
                                    <div class="fc-airline-meta">
                                        <span v-if="leg.segments && leg.segments[0]" class="fc-fi-item fc-fi-fn">
                                            <i class="fa fa-hashtag"></i>{{ leg.segments[0].full_flight_number }}
                                        </span>
                                        <span v-if="leg.segments && leg.segments[0] && leg.segments[0].aircraft_name" class="fc-fi-item">
                                            <i class="fa fa-plane-up"></i>{{ leg.segments[0].aircraft_name }}
                                        </span>
                                        ...
                                        <span v-if="leg.segments && leg.segments[0] && leg.segments[0].fare_info && leg.segments[0].fare_info.seats_available <= 9" class="fc-fi-item fc-fi-seats">
                            ...
                            <div class="fc-route">
                                ...
                                <div class="fc-mid">
                                    ...
                                    <div class="fc-line">
                                        <span class="fc-dot fc-dot-dep"></span>
                                        <span class="fc-dashes"></span>
                                        <span class="fc-plane-icon"><i class="fa fa-plane"></i></span>
                                        <span class="fc-dashes"></span>
                                        <span class="fc-dot fc-dot-arr"></span>
                                    </div>
                            ...
                            <div v-if="!leg.is_direct && leg.stops_detail && leg.stops_detail.length" class="fc-layovers">
                                <div v-for="(stop, si) in leg.stops_detail" :key="si" class="fc-layover-pill">
                                    ...
                                    {{ stop.airport_code }} · {{ stop.layover_formatted }}
                            ...
                            <div class="fc-tags">
                                ...
                                <span v-if="flight.refundable" class="fc-tag fc-ref"><i class="fa fa-check-circle"></i> Refundable</span>
            ...
            <!-- RIGHT: price panel — spans full card height -->
            <div class="fc-right">

                <!-- Approx note -->
                <div v-if="flight.price && flight.price.tax_note" class="fc-approx">
                    {{ flight.price.tax_note }}
                </div>
                ...
                <div class="fc-price-breakdown">
                    ...
                    <div class="fc-pb-row">
                        <span>Base Fare</span>
                        <span class="fc-pb-val">৳{{ formatPrice(flight.price && flight.price.api_base_fare) }}</span>
                    </div>
                    <!-- ② Tax -->
                    <div class="fc-pb-row">
                        <span>+ Tax</span>
                        <span class="fc-pb-val">৳{{ formatPrice(flight.price && flight.price.api_tax) }}</span>
                    </div>
                    <!-- ③ Gross = Base + Tax -->
                    <div class="fc-pb-row fc-pb-subtotal">
                        <span>= Gross Fare</span>
                        <span>৳{{ formatPrice(grossFare) }}</span>
                    </div>
                    ...
                    <div v-if="flight.charges_details && +flight.charges_details.service_charge" class="fc-pb-row fc-pb-charge">
                        ...
                        <span class="fc-pb-val">৳{{ formatPrice(flight.charges_details.service_charge) }}</span>
                    </div>
                    <!-- ⑥ Subtotal = Gross + AIT + Svc -->
                    <div v-if="hasCharges" class="fc-pb-row fc-pb-subtotal">
                        <span>= Subtotal</span>
                        <span>৳{{ formatPrice(subtotalBeforeDiscount) }}</span>
                    </div>

                    <!-- ⑦ Segment Discount -->
                    <div v-if="segmentDiscount > 0" class="fc-pb-row fc-pb-disc">
                        <span>
                            <i class="fa fa-tag"></i> Segment Disc.
                            <span v-if="flight.charges_details.segment_discount_label" class="fc-pb-pct">
                                ({{ flight.charges_details.segment_discount_label }})
                            </span>
                        </span>
                    ...
                    <!-- ⑧ Other Discount -->
                    <div v-if="otherDiscount > 0" class="fc-pb-row fc-pb-disc">
                        <span>
                            <i class="fa fa-tag"></i> Discount
                            <span v-if="flight.charges_details.flight_discount_label" class="fc-pb-pct">
                                ({{ flight.charges_details.flight_discount_label }})
                            </span>
                        </span>
                ...
                <div class="fc-total">
                    ...
                    <span class="fc-total-price">৳{{ formatPrice(flight.price && flight.price.total) }}</span>
                ...
                <div class="fc-btns">
                    ...
                    <div class="fc-actions">
                        <button @click="openModal" class="fc-detail-btn">
                            <i class="fa fa-list"></i> Details
                        </button>
                        <button @click="copyFlightDetails" class="fc-copy-btn" :class="{ copied: isCopied }">
        ...
        <div class="fc-mobile-price">
            <div class="fc-mp-left" @click="showPriceSheet = !showPriceSheet">
                <div class="fc-mp-price-row">
                    <span class="fc-mp-price">৳{{ formatPrice(flight.price && flight.price.total) }}</span>
                    <span class="fc-mp-pax">/ person</span>
                </div>
                <div class="fc-mp-breakdown-hint">
                    <i class="fa fa-receipt"></i> View breakdown
                    <i class="fa" :class="showPriceSheet ? 'fa-chevron-down' : 'fa-chevron-up'"></i>
                </div>
            </div>
            <div class="fc-mp-right">
                <button @click="openModal" class="fc-mp-detail" title="Flight Details">
                    <i class="fa fa-list"></i>
                </button>
                <button @click="copyFlightDetails" class="fc-mp-copy" :class="{ copied: isCopied }">
                    <i :class="isCopied ? 'fa fa-check' : 'fa fa-copy'"></i>
                </button>
        ...
        <!-- ── MOBILE PRICE BREAKDOWN SHEET ── -->
        <transition name="fc-sheet">
            <div v-if="showPriceSheet" class="fc-price-sheet">
                <div class="fc-ps-title">
                    <i class="fa fa-receipt"></i> Price Breakdown
                    <button @click="showPriceSheet = false" class="fc-ps-close"><i class="fa fa-times"></i></button>
                </div>

                <!-- ① Base + Tax → Gross -->
                <div class="fc-ps-row">
                    <span>Base Fare</span>
                    <span>৳{{ formatPrice(flight.price && flight.price.api_base_fare) }}</span>
                </div>
                <div class="fc-ps-row">
                    <span>+ Tax</span>
                    <span>৳{{ formatPrice(flight.price && flight.price.api_tax) }}</span>
                </div>
                <div class="fc-ps-row fc-ps-sub">
                    <span>= Gross Fare</span>
                    <span>৳{{ formatPrice(grossFare) }}</span>
                </div>
                ...
                <template v-if="hasCharges">
                    <div v-if="flight.charges_details && +flight.charges_details.ait_amount" class="fc-ps-row fc-ps-charge">
                        <span>+ AIT<span v-if="flight.charges_details.ait_charge_percentage" class="fc-pb-pct"> ({{ flight.charges_details.ait_charge_percentage }}%)</span></span>
                        <span>৳{{ formatPrice(flight.charges_details.ait_amount) }}</span>
                    </div>
                    <div v-if="flight.charges_details && +flight.charges_details.service_charge" class="fc-ps-row fc-ps-charge">
                        <span>+ Service Charge</span>
                        <span>৳{{ formatPrice(flight.charges_details.service_charge) }}</span>
                    </div>
                    <div class="fc-ps-row fc-ps-sub">
                        <span>= Subtotal</span>
                        <span>৳{{ formatPrice(subtotalBeforeDiscount) }}</span>
                ...
                <!-- ③ Discounts -->
                <div v-if="segmentDiscount > 0" class="fc-ps-row fc-ps-disc">
                    <span><i class="fa fa-tag"></i> Segment Discount</span>
                    <span>-৳{{ formatPrice(segmentDiscount) }}</span>
                </div>
                <div v-if="otherDiscount > 0" class="fc-ps-row fc-ps-disc">
                    <span><i class="fa fa-tag"></i> Discount</span>
                    <span>-৳{{ formatPrice(otherDiscount) }}</span>
                </div>

                <!-- ④ You Pay -->
                <div class="fc-ps-total">
                    <span>You Pay</span>
                    <span>৳{{ formatPrice(flight.price && flight.price.total) }}</span>
                </div>
                ...
                <div class="fc-modal">
                        ...
                        <div class="fc-mh-info">
                            ...
                            <div>
                                <div class="fc-mh-route">
                                    {{ legs[0] && legs[0].departure && legs[0].departure.airport_code }}
                                    <i class="fa fa-arrow-right"></i>
                                    {{ legs[legs.length-1] && legs[legs.length-1].arrival && legs[legs.length-1].arrival.airport_code }}
                                </div>
                                <div class="fc-mh-sub">
                    ...
                    <div class="fc-tabs">
                        <button v-for="tab in modalTabs" :key="tab.key"
                                ...
                                class="fc-tab" :class="{ active: activeModalTab === tab.key }">
                            <i :class="tab.icon"></i>
                            <span>{{ tab.label }}</span>
                        </button>
                    </div>
                    ...
                    <div class="fc-mb">
                        ...
                        <div v-if="activeModalTab === 'flight'">
                            <div v-for="(leg, li) in legs" :key="'ml-'+li" class="fc-mb-section">
                                <div class="fc-mb-heading">
                                    <i :class="getLegIcon(leg.leg_type)"></i>
                                    {{ getLegLabel(li) }}: {{ leg.departure && leg.departure.airport_code }} → {{ leg.arrival && leg.arrival.airport_code }}
                                </div>
                                <div v-for="(seg, si) in (leg.segments || [])" :key="'ms-'+li+'-'+si" class="fc-seg-card">
                                    <div class="fc-seg-head">
                                        ...
                                        <div class="fc-seg-head-info">
                                            <div class="fc-seg-fn">{{ seg.full_flight_number }}</div>
                                            <div class="fc-seg-carrier">{{ seg.carrier_name }}</div>
                                        </div>
                                        <div class="fc-seg-badges">
                                                <span class="fc-seg-badge fc-sb-ac">
                                                    <i class="fa fa-plane-up"></i> {{ seg.aircraft_name || seg.aircraft || 'N/A' }}
                                                </span>
                                            <span v-if="seg.fare_info" class="fc-seg-badge fc-sb-cabin">
                                                    {{ seg.fare_info.cabin_name }}
                                                    <span v-if="seg.fare_info.booking_code" class="fc-sb-code">· {{ seg.fare_info.booking_code }}</span>
                                                </span>
                                            <span v-if="seg.fare_info && seg.fare_info.seats_available" class="fc-seg-badge" :class="seg.fare_info.seats_available <= 5 ? 'fc-sb-hot' : 'fc-sb-seats'">
                                                    <i class="fa fa-fire" v-if="seg.fare_info.seats_available <= 5"></i>
                                                    <i class="fa fa-chair" v-else></i>
                                                    {{ seg.fare_info.seats_available }} seats
                                                </span>
                                        </div>
                                    </div>
                                    <div class="fc-seg-tl">
                                        ...
                                        <div class="fc-tl-mid">
                                            <div class="fc-tl-line"></div>
                                            <div class="fc-tl-dur">{{ seg.duration_formatted }}</div>
                                        ...
                                        <div class="fc-tl-pt">
                                            ...
                                            <div>
                                                <div class="fc-tl-time">
                                                    {{ seg.arrival && seg.arrival.time_12h }}
                                                    <span v-if="seg.arrival && seg.arrival.date_adjustment > 0" class="fc-next-day">+{{ seg.arrival.date_adjustment }}</span>
                                                </div>
                                    ...
                                    <!-- Extra segment info row -->
                                    <div class="fc-seg-meta">
                                            <span v-if="seg.is_codeshare" class="fc-sm-item fc-sm-cs">
                                                <i class="fa fa-code-branch"></i> Codeshare — operated by {{ seg.operating_carrier_name }}
                                            </span>
                                        <span v-if="seg.meal_description" class="fc-sm-item">
                                                <i class="fa fa-utensils"></i> {{ seg.meal_description }}
                                            </span>
                                        <span v-if="seg.eTicketable" class="fc-sm-item">
                                                <i class="fa fa-ticket"></i> E-Ticket
                                            </span>
                                        <span v-if="seg.fare_info && seg.fare_info.fare_basis_code" class="fc-sm-item fc-sm-fbc">
                                                <i class="fa fa-barcode"></i> {{ seg.fare_info.fare_basis_code }}
                                            </span>
                                    </div>
                                    <div v-if="seg.layover_after" class="fc-layover-bar" :class="{ overnight: seg.layover_after.is_overnight }">
                                        <i class="fa fa-clock"></i>
                                        Layover {{ seg.layover_after.formatted }} · {{ seg.layover_after.airport_code }}
                        ...
                        <div v-if="activeModalTab === 'fare'">
                            ...
                            <div class="fc-table-wrap">
                                <table class="fc-table">
                                    ...
                                    <tr v-for="(p,i) in (flight.passengers || [])" :key="i">
                                        ...
                                        <td>৳{{ formatPrice(p.base_fare) }}</td>
                                        <td>৳{{ formatPrice(p.tax_amount) }}</td>
                            ...
                            <div class="fc-calc">
                                ...
                                <div class="fc-cr fc-cr-sub">
                                    <span>= Gross Fare</span>
                                    <span>৳{{ formatPrice(grossFare) }}</span>
                                </div>

                                <!-- ② AIT -->
                                <template v-if="flight.charges_details && +flight.charges_details.ait_amount">
                                    <div class="fc-cr fc-cr-add">
                <span>
                    + AIT
                    <span v-if="flight.charges_details.ait_charge_percentage" class="fc-pb-pct">
                        ({{ flight.charges_details.ait_charge_percentage }}%)
                    </span>
                </span>
                                        <span>৳{{ formatPrice(flight.charges_details.ait_amount) }}</span>
                                    </div>
                                ...
                                <!-- ③ Service Charge -->
                                <template v-if="flight.charges_details && +flight.charges_details.service_charge">
                                    <div class="fc-cr fc-cr-add">
                                        <span>+ Service Charge</span>
                                        <span>৳{{ formatPrice(flight.charges_details.service_charge) }}</span>
                                    </div>
                                ...
                                <!-- ④ Subtotal -->
                                <div v-if="hasCharges" class="fc-cr fc-cr-sub">
                                    <span>= Subtotal</span>
                                    <span>৳{{ formatPrice(subtotalBeforeDiscount) }}</span>
                                </div>

                                <!-- ⑤ Segment Discount -->
                                <div v-if="segmentDiscount > 0" class="fc-cr fc-cr-disc">
                                    <span>
                                        <i class="fa fa-tag"></i> Segment Disc.
                                        <span v-if="flight.charges_details && flight.charges_details.segment_discount_label" class="fc-pb-pct">
                                            ({{ flight.charges_details.segment_discount_label }})
                                        </span>
                                    </span>
                                    <span>-৳{{ formatPrice(segmentDiscount) }}</span>
                                ...
                                <!-- ⑥ Flight Discount -->
                                <div v-if="otherDiscount > 0" class="fc-cr fc-cr-disc">
                                    <span>
                                        <i class="fa fa-tag"></i> Discount
                                        <span v-if="flight.charges_details && flight.charges_details.flight_discount_label" class="fc-pb-pct">
                                            ({{ flight.charges_details.flight_discount_label }})
                                        </span>
                                    </span>
                                    <span>-৳{{ formatPrice(otherDiscount) }}</span>
                                ...
                                <div class="fc-cr fc-cr-total">
                                    ...
                                    <span><strong>৳{{ formatPrice(flight.price && flight.price.total) }}</strong></span>
                                </div>
                        ...
                        <div v-if="activeModalTab === 'baggage'">
                            ...
                            <div class="fc-bag-cards">
                                <div v-for="(p,i) in (flight.passengers || [])" :key="i" class="fc-bag-card">
                                    <div class="fc-bag-type">{{ p.type_label }} × {{ p.count }}</div>
                                    <div class="fc-bag-val"><i class="fa fa-suitcase"></i> {{ formatBaggage(p.baggage) }}</div>
                                </div>
                            </div>
                        ...
                        <div v-if="activeModalTab === 'penalty'">
                            <div class="fc-refund-banner" :class="flight.refundable ? 'green' : 'red'">
                                <i :class="flight.refundable ? 'fa fa-check-circle' : 'fa fa-times-circle'"></i>
                                <div>
                                    <strong>{{ flight.refundable ? 'Refundable Fare' : 'Non-Refundable Fare' }}</strong>
                                    <p>{{ flight.refundable ? 'Refund available subject to airline fees.' : 'This fare cannot be refunded.' }}</p>
                                </div>
                            </div>
                            <div class="fc-penalty-grid">
                                <div class="fc-pen-card">
                                    <div class="fc-pen-head red"><i class="fa fa-times-circle"></i> Cancellation</div>
                                    <div class="fc-pen-row"><span>Before Departure</span><span class="red">Airline Fee + Tax</span></div>
                                    <div class="fc-pen-row"><span>No-Show</span><span class="red">100% Forfeit</span></div>
                                </div>
                                <div class="fc-pen-card">
                                    <div class="fc-pen-head orange"><i class="fa fa-calendar-alt"></i> Date Change</div>
                                    <div class="fc-pen-row"><span>Before Departure</span><span class="orange">Fare Diff + Fee</span></div>
                                    <div class="fc-pen-row"><span>After Departure</span><span class="red">Not Allowed</span></div>
                                </div>
                            </div>
                            <div v-if="flight.last_ticket_date" class="fc-notice fc-notice-red mt-12">
                                <i class="fa fa-clock"></i> Last Ticketing: {{ formatDate(flight.last_ticket_date) }}
                            </div>
                        </div>
                            ...
                            <div v-if="flight.taxes_breakdown && flight.taxes_breakdown.length" class="fc-table-wrap">
                                <table class="fc-table">
                                    ...
                                    <tr v-for="(tax,i) in flight.taxes_breakdown" :key="i">
                                        <td><span class="fc-tax-code">{{ tax.code }}</span></td>
                                        <td class="fc-tax-desc">{{ tax.description }}</td>
                    ...
                    <div class="fc-mf">
                        <div class="fc-mf-price">
                            <span>Total</span>
                            <strong>৳{{ formatPrice(flight.price && flight.price.total) }}</strong>
                        </div>
                        <div class="fc-mf-btns">
                            <button @click="closeModal" class="fc-mf-sec">Close</button>
                            <button @click="bookFlight" class="fc-mf-pri"><i class="fa fa-bolt"></i> Book Now</button>
                        </div>
                    </div>
        ...
        <transition name="fc-modal-fade">
            <div v-if="showAirArabiaModal" class="fc-modal-overlay">
                <div class="fc-modal">
                    ...
                    <!-- Header -->
                    <div class="fc-mh" style="background: linear-gradient(135deg,#92400e,#d97706)">
                        <div class="fc-mh-info">
                            <i class="fa fa-info-circle" style="color:white;font-size:20px;margin-right:10px"></i>
                            <div>
                                <div class="fc-mh-route">মূল্য নিশ্চিত করুন</div>
                                <div class="fc-mh-sub" style="font-size:11px;opacity:.85">
                            <span v-for="(p,i) in (airArabiaModalData?.data?.passengers||[])" :key="i" class="fc-mh-pax">
                                {{ p.count }}× {{ p.type_label }}
                            </span>
                                </div>
                            </div>
                        </div>
                    ...
                    <!-- Tabs -->
                    <div class="fc-tabs">
                        <button @click="airArabiaModalTab='price'"   class="fc-tab" :class="{active:airArabiaModalTab==='price'}">
                            <i class="fa fa-calculator"></i><span>Price</span>
                        </button>
                        <button @click="airArabiaModalTab='tax'"     class="fc-tab" :class="{active:airArabiaModalTab==='tax'}">
                            <i class="fa fa-receipt"></i><span>Tax</span>
                        </button>
                        <button @click="airArabiaModalTab='pax'"     class="fc-tab" :class="{active:airArabiaModalTab==='pax'}">
                            <i class="fa fa-users"></i><span>Passengers</span>
                        </button>
                        <button @click="airArabiaModalTab='bundle'"  class="fc-tab" :class="{active:airArabiaModalTab==='bundle'}">
                            <i class="fa fa-boxes"></i><span>Bundle</span>
                        </button>
                    </div>
                    ...
                    <div class="fc-mb" v-if="airArabiaModalData">
                        ...
                        <div v-if="airArabiaModalTab==='price'">
                            <div class="fc-notice fc-notice-yellow mb-12">
                                ...
                                Search এ শুধু Base Fare ছিল। Air Arabia confirmed price এ Tax ও fees যোগ হয়েছে।
                            </div>
                            <div class="fc-calc">
                                <div class="fc-cr fc-cr-base">
                                    <span>Search Price (Tax ছাড়া)</span>
                                    <span class="strike">৳{{ formatPrice(airArabiaModalData.old_price) }}</span>
                                </div>
                                <div class="fc-cr fc-cr-base">
                                    <span>Base Fare</span>
                                    <span>৳{{ formatPrice(airArabiaModalData.data.price.api_base_fare) }}</span>
                                </div>
<!--                                <div class="fc-cr fc-cr-base">-->
...
<!--                                </div>-->
                                <div class="fc-cr fc-cr-add">
                                    <span>+ Tax</span>
                                    <span>৳{{ formatPrice(airArabiaModalData.data.price.api_tax) }}</span>
                                </div>
                                <div class="fc-cr fc-cr-sub">
                                    <span>= Gross Fare</span>
                                    <span>৳{{ formatPrice(airArabiaModalData.data.price.api_subtotal) }}</span>
                                </div>
                                ...
                                <div class="fc-cr fc-cr-add" v-if="airArabiaModalData.data.charges_details && +airArabiaModalData.data.charges_details.service_charge">
                                    <span>+ Service Charge</span>
                                    <span>৳{{ formatPrice(airArabiaModalData.data.charges_details.service_charge) }}</span>
                                </div>
                                <div class="fc-cr fc-cr-sub" v-if="airArabiaModalData.data.price.subtotal_before_discount">
                                    <span>= Subtotal</span>
                                    <span>৳{{ formatPrice(airArabiaModalData.data.price.subtotal_before_discount) }}</span>
                                </div>
                                <div class="fc-cr fc-cr-disc" v-if="+airArabiaModalData.data.price.flight_discount > 0">
                                    <span><i class="fa fa-tag"></i> Flight Discount</span>
                                    <span>-৳{{ formatPrice(airArabiaModalData.data.price.flight_discount) }}</span>
                                </div>
                                <div class="fc-cr fc-cr-disc" v-if="+airArabiaModalData.data.price.segment_discount > 0">
                                    <span><i class="fa fa-ticket"></i> Segment Discount</span>
                                    <span>-৳{{ formatPrice(airArabiaModalData.data.price.segment_discount) }}</span>
                                </div>
                                <div class="fc-cr fc-cr-total">
                                    <span><strong>Confirmed Price</strong></span>
                                    <span><strong>৳{{ formatPrice(airArabiaModalData.data.price.total) }}</strong></span>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-if="airArabiaModalTab==='tax'">
                            ...
                            <div class="fc-table-wrap">
                                <table class="fc-table">
                                    ...
                                    <tfoot>
                                    <tr class="fc-tbl-total">
                                        <td colspan="2"><strong>Total Tax / Adult</strong></td>
                                        <td class="right"><strong>৳{{ formatPrice(airArabiaModalData.data.taxes_breakdown && airArabiaModalData.data.taxes_breakdown.reduce((s,t)=>s+t.amount,0)) }}</strong></td>
                                    </tr>
                                    </tfoot>
                                </table>
                            </div>
                            <div class="fc-notice fc-notice-blue mt-12">
                                <i class="fa fa-info-circle"></i>
                                Total tax (all passengers) = ৳{{ formatPrice(airArabiaModalData.data.price.api_tax) }}
                            </div>
                        </div>
...
<!--                                        <th>Fees</th>-->
                                    ...
                                    <tr v-for="(p,i) in (airArabiaModalData.data.passengers||[])" :key="i">
                                        <td><strong>{{ p.type_label }}</strong></td>
                                        <td class="center">{{ p.count }}</td>
                                        <td>৳{{ formatPrice(p.equivalent_amount) }}</td>
                                        <td>৳{{ formatPrice(p.tax_amount) }}</td>
<!--                                        <td>৳{{ formatPrice(p.fees_amount) }}</td>-->
                                        <td>৳{{ formatPrice(p.total_fare) }}</td>
                                        <td><strong>৳{{ formatPrice(p.total_fare * p.count) }}</strong></td>
                                    </tr>
                                    </tbody>
                                </table>
                            ...
                            <!-- Per passenger charges breakdown -->
                            <div v-for="(bd,i) in (airArabiaModalData.data.passenger_price_breakdown||[])" :key="'pbd-'+i" class="mt-12">
                                <div class="fc-mb-heading">
                                    <i class="fa fa-user"></i>
                                    {{ bd.passenger_type }} × {{ bd.passenger_count }} — Charges
                                </div>
                                <div class="fc-calc">
                                    <div class="fc-cr fc-cr-base">
                                        <span>API Total</span>
                                        <span>৳{{ formatPrice(bd.per_pax.api_total) }}</span>
                                    </div>
                                    <div class="fc-cr fc-cr-add" v-if="+bd.per_pax.ait_amount">
                                        <span>+ AIT</span>
                                        <span>৳{{ formatPrice(bd.per_pax.ait_amount) }}</span>
                                    </div>
                                    <div class="fc-cr fc-cr-add" v-if="+bd.per_pax.service_charge">
                                        <span>+ Service Charge</span>
                                        <span>৳{{ formatPrice(bd.per_pax.service_charge) }}</span>
                                    </div>
                                    <div class="fc-cr fc-cr-disc" v-if="+bd.per_pax.user_discount">
                                        <span><i class="fa fa-tag"></i> Discount</span>
                                        <span>-৳{{ formatPrice(bd.per_pax.user_discount) }}</span>
                                    </div>
                                    <div class="fc-cr fc-cr-disc" v-if="+bd.per_pax.user_seg_discount">
                                        <span><i class="fa fa-ticket"></i> Segment Discount</span>
                                        <span>-৳{{ formatPrice(bd.per_pax.user_seg_discount) }}</span>
                                    </div>
                                    <div class="fc-cr fc-cr-total">
                                        <span><strong>Payable / Pax</strong></span>
                                        <span><strong>৳{{ formatPrice(bd.per_pax.user_payable) }}</strong></span>
                                    </div>
                                </div>
                            </div>
                        </div>
                        ...
                        <div v-if="airArabiaModalTab==='bundle'">
                            <div class="fc-notice fc-notice-blue mb-12">
                                <i class="fa fa-info-circle"></i>
                                Bundle add-ons are optional. Book করার পর bundle select করতে পারবেন।
                            </div>
                            <div v-for="(bg,gi) in (airArabiaModalData.data.bundle_options||[])" :key="'bg-'+gi" class="mb-12">
                                <div class="fc-mb-heading">
                                    <i class="fa fa-route"></i> {{ bg.ond }}
                                </div>
                                <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(140px,1fr));gap:8px;">
                                    <div v-for="(b,bi) in bg.bundles" :key="'b-'+gi+'-'+bi"
                                         style="border:1px solid var(--fc-border);border-radius:10px;overflow:hidden;">
                                        <div style="padding:8px 10px;background:var(--fc-bg);border-bottom:1px solid var(--fc-border);">
                                            <div style="font-weight:700;font-size:13px;color:var(--fc-blue);">{{ b.name }}</div>
                                            <div style="font-size:11px;color:var(--fc-muted);">{{ b.booking_class }}</div>
                                        </div>
                                        <div style="padding:8px 10px;">
                                            <div style="font-size:15px;font-weight:700;color:#059669;margin-bottom:6px;">
                                                +৳{{ formatPrice(b.fee_per_pax) }}/pax
                                            </div>
                                            <div v-for="(svc,si) in b.services" :key="si"
                                                 style="font-size:11px;color:#374151;margin-bottom:2px;display:flex;align-items:center;gap:4px;">
                                                <i class="fa fa-check" style="color:#16a34a;font-size:10px;"></i>{{ svc }}
                                            </div>
                                        </div>
                                    </div>
                                </div>
                    ...
                    <div class="fc-mf">
                        <div class="fc-mf-price">
                            ...
                            <strong>৳{{ airArabiaModalData ? formatPrice(airArabiaModalData.data.price.total) : 0 }}</strong>
                        </div>
                        <div class="fc-mf-btns">
                            <button @click="cancelAirArabia" class="fc-mf-sec">বাতিল</button>
                            <button @click="confirmAirArabia" class="fc-mf-pri">
                                <i class="fa fa-check"></i> বুক করুন
                            </button>
                        </div>
                    </div>
...
export default {
    ...
    computed: {
        ...
        // ── Price calculation ──
        grossFare() {
            return (Number(this.flight.price?.api_base_fare) || 0) +
                (Number(this.flight.price?.api_tax) || 0);
        },
        hasCharges() {
            return (Number(this.flight.charges_details?.ait_amount) || 0) +
                (Number(this.flight.charges_details?.service_charge) || 0) > 0;
        },
        subtotalBeforeDiscount() {
            return this.grossFare +
                (Number(this.flight.charges_details?.ait_amount) || 0) +
                (Number(this.flight.charges_details?.service_charge) || 0);
        },
        segmentDiscount() {
            return Number(this.flight.charges_details?.segment_discount_total) || 0;
        },
        otherDiscount() {
            return Number(this.flight.flight_discount_details?.flight_discount_amount) || 0;
        },
        hasDiscounts() {
            return this.segmentDiscount > 0 || this.otherDiscount > 0;
        },
        totalDiscount() {
            return this.segmentDiscount + this.otherDiscount;
        },
    },
    methods: {
        getLegLabel(i) {
        ...
        getCabinClass(leg)  { return leg.segments?.[0]?.fare_info?.cabin_name || 'Economy'; },
        ...
        cancelAirArabia()  { if (confirm('বাতিল করবেন?')) { this.showAirArabiaModal = false; this.airArabiaModalData = null; } },
        ...
        copyFlightDetails() {
            ...
            if (this.flight.price) {
                ...
                lines.push(`Refundable: ${this.flight.refundable ? '✅ Yes' : '❌ No'}`);
            }
        ...
        bookFlight(event) {
            if (!window.isLoggedIn) {
                try { new bootstrap.Modal(document.getElementById('login')).show(); } catch { document.querySelector('[data-bs-target="#login"]')?.click(); }
                return;
            }
            ...
            fetch('flight/flightToCart', {
                ...
                if (data.success) { window.location.href = data.redirect_url; return; }
                if (data.air_arabia) { this.airArabiaModalData = data; this.showAirArabiaModal = true; if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fa fa-bolt"></i> Book Now'; } return; }
                if (data.price_changed) { alert('মূল্য পরিবর্তন হয়েছে।\nপুরনো: ৳' + data.old_price + '\nনতুন: ৳' + data.new_api_price); window.history.back(); return; }
                alert('Error: ' + data.message);
                if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fa fa-bolt"></i> Book Now'; }
            }).catch(() => { alert('Something went wrong!'); if (btn) { btn.disabled = false; btn.innerHTML = '<i class="fa fa-bolt"></i> Book Now'; } });
        }
    }
};
</script>
...
/* ── MAIN ── */
/* ── BODY: legs left + price right ── */
.fc-body {
    display: flex;
    align-items: stretch;   /* ✅ right panel full height */
}
...
.fc-plane-icon { font-size: 14px; color: var(--fc-blue); transform: rotate(0deg); }
.fc-stops-badge {
    font-size: 10px;
    font-weight: 700;
    padding: 2px 8px;
...
/* ── Flight info strip CSS (used in fc-airline-meta) ── */
.fc-fi-item {
    display: inline-flex;
    align-items: center;
    gap: 3px;
...
.fc-price-breakdown {
    ...
    flex-direction: column;
    gap: 2px;
    flex: 1;                /* ✅ takes available space, pushes total+btns down */
}

.fc-approx {
    font-size: 11px;
    font-weight: 700;
    background: linear-gradient(90deg, #0a3afa, #f80404, #f1bf09);
    -webkit-background-clip: text;
    background-clip: text;
    color: transparent;
    background-size: 200% auto;
    animation: fc-grad 4s linear infinite;
    text-align: center;
    margin-bottom: 6px;
}
...
.fc-ps-row:last-of-type { border-bottom: none; }
.fc-ps-sub {
    font-weight: 700;
    color: var(--fc-text);
    background: #f8fafc;
...
.fc-ps-disc   { color: var(--fc-green); font-weight: 600; }
.fc-ps-total {
    display: flex;
    justify-content: space-between;
    align-items: center;
...
@media (min-width: 640px) {
    .fc-modal-overlay { align-items: center; padding: 16px; }
}
...
@media (min-width: 640px) {
    .fc-modal { border-radius: 16px; max-height: 88vh; }
}
...
@media (min-width: 640px) {
    .fc-modal-fade-enter-from, .fc-modal-fade-leave-to { transform: scale(.96); }
}
...
.fc-mh {
    ...
    padding: 14px 16px;
    background: linear-gradient(135deg, var(--fc-blue2), var(--fc-blue));
    color: #fff;
    flex-shrink: 0;
}
...
.fc-mh-sub { display: flex; flex-wrap: wrap; gap: 5px; margin-top: 3px; }
...
.fc-mh-close:hover { background: rgba(255,255,255,.35); }
...
/* Baggage */
.fc-bag-cards { display: grid; grid-template-columns: repeat(auto-fill, minmax(140px,1fr)); gap: 8px; }
.fc-bag-card { border: 1px solid var(--fc-border); border-radius: 8px; padding: 10px; text-align: center; }
...
@media (max-width: 900px) {
    .fc-right { flex: 0 0 200px; padding: 12px; }
    .fc-time  { font-size: 20px; }
    .fc-total-price { font-size: 20px; }
}
...
@media (max-width: 640px) {
    ...
    .fc-main  { padding: 12px; gap: 0; }

    /* Hide desktop price panel */
    ...
    /* Smaller route on mobile */
    .fc-time { font-size: 18px; }
    .fc-code { font-size: 13px; }
    .fc-city { font-size: 10px; max-width: 60px; }
    .fc-endpoint { min-width: 52px; }
    .fc-route { gap: 4px; }
    ...
    .fc-seg-tl { flex-direction: column; gap: 10px; }
    .fc-tl-mid { width: 100%; flex-direction: row; gap: 8px; }
    .fc-mf { flex-direction: column; gap: 10px; }
    .fc-mf-btns { display: grid; grid-template-columns: 1fr 1fr; width: 100%; }
    .fc-mf-price { align-items: center; }
...
@media (max-width: 380px) {
    .fc-main { padding: 10px; }
    .fc-time { font-size: 16px; }
    .fc-endpoint { min-width: 44px; }
    .fc-tags { gap: 4px; }
    .fc-tab span { display: none; }
    .fc-tab i { font-size: 16px; }
    .fc-tab { padding: 10px 6px; }
}
</style>
