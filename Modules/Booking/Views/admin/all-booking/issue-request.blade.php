    ...
    <div class="container-fluid px-3">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
            <h1 class="title-bar mb-0">
                <i class="fa fa-check-circle text-success"></i> {{ __('Issue Request Bookings') }}
                <span class="badge badge-success ml-2">{{ $rows->total() }} {{ __('bookings') }}</span>
            </h1>
            <div>
                <a href="{{ route('all-booking.issue-request') }}" class="btn btn-outline-secondary btn-sm">
        ...
        <div class="card shadow-sm mb-3">
            <div class="card-body py-2 px-3">
                <form method="GET" action="{{ route('all-booking.issue-request') }}" id="filter-form">
                        ...
                        <div class="col-auto">
                            ...
                            <div class="d-flex gap-1">
                                ...
                                @if(!empty($dateFrom) || !empty($dateTo) || !empty($searchTerm))
                                    <a href="{{ route('all-booking.issue-request') }}" class="btn btn-secondary btn-sm">
                    ...
                    <table id="bookings-table" class="table table-hover table-bordered table-sm mb-0" style="width:100%">
                        ...
                        @forelse($rows as $i => $booking)
                            ...
                            <tr>
                                ...
                                {{-- Status --}}
                                <td>
                                    <span class="badge badge-issue_request" style="font-size:11px">
                                        <i class="fa fa-check-circle"></i> issue_request
                                    </span>
                                    @if($hasTicket)
                            ...
                            <tr>
                                <td colspan="14" class="text-center py-4">
                                    <div class="text-muted">
                                        ...
                                        {{ __('No issue_request bookings found') }}
                                    </div>
                                </td>
                            </tr>
                        @endforelse
...
@push('css')
    ...
    <style>
        .badge-issue_request {
            background: linear-gradient(135deg, #1900ff, #1001df);
            color: #fff;
            padding-left: 16px;
            position: relative;
        }
        .badge-issue_request::before {
            content: '';
            position: absolute;
            left: 5px;
            top: 50%;
...
$(document).ready(function() {
    ...
    $(document).on('click', '.btn-pnr-edit', function(e) {
        ...
        $('#pnr_edit_status').val($(this).data('status') || 'issue_request');
