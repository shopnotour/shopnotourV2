    ...
    <div class="dashboard bravo_user_profile p-0" data-x="dashboard" data-x-toggle="-is-sidebar-open">
        ...
        <div class="dashboard__main">
            <div class="dashboard__content bg-light-2">
                ...
                {{-- ── B2B Pending Approval Notice ── --}}

{{--                {{ dd($pendingB2B ?? 'NOT SET') }}--}}
                @if(!empty($pendingB2B))
                    <div style="background:#fffbeb;border:1.5px solid #f59e0b;border-left:5px solid #f59e0b;border-radius:10px;padding:14px 20px;margin:20px 20px 0;display:flex;align-items:flex-start;gap:12px;">
                        <svg width="20" height="20" fill="none" viewBox="0 0 24 24" stroke="#d97706" stroke-width="2" style="flex-shrink:0;margin-top:2px;">
                            <path stroke-linecap="round" stroke-linejoin="round" d="M12 9v2m0 4h.01M10.29 3.86L1.82 18a2 2 0 001.71 3h16.94a2 2 0 001.71-3L13.71 3.86a2 2 0 00-3.42 0z"/>
                        </svg>
                        <div>
                            <div style="font-size:14px;font-weight:700;color:#92400e;margin-bottom:3px;">
                                {{ __('B2B Account Approval Pending') }}
                            </div>
                            <div style="font-size:13px;color:#b45309;line-height:1.55;">
                                {{ __('Your corporate (B2B) account request is under review. Once approved, you will receive Agent access and be notified via email.') }}
                            </div>
                        </div>
                    </div>
                @endif
