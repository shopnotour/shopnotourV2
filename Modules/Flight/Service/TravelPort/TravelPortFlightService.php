...
class TravelPortFlightService
{
    ...
    public function search($searchData)
    {
        ...
        if (is_array($travelPortResponse)) {
            if (!($travelPortResponse['success'] ?? true)) {
                Log::warning('TravelPort API failed', [
                    'error' => $travelPortResponse['error'] ?? 'Unknown error',
                ]);
            ...
            $xmlString = $travelPortResponse['data']
                ?? $travelPortResponse['xml']
                ?? $travelPortResponse['body']
                ?? null;
        } else {
            $xmlString = $travelPortResponse;
        }
        ...
        if (empty($xmlString)) {
            // ✅ log যোগ করো
            Log::warning('TravelPort empty response');
        ...
        // ✅ exception handle করো
        try {
            return $this->parseFlightSearchResponse($xmlString);
        } catch (\Exception $e) {
            Log::warning('TravelPort response parse failed', [
                'error' => $e->getMessage(),
            ]);

            return [
                'success' => false,
                'error' => 'TravelPort response parse failed',
                'flights' => [],
            ];
        }
    }
    ...
    public function parseFlightSearchResponse(string|array $xmlResponse): array
    {
        ...
        if (!is_string($xmlResponse) || trim($xmlResponse) === '') {
            Log::warning('TravelPort invalid XML response', [
                'type' => gettype($xmlResponse),
            ]);
            throw new \Exception('Invalid SOAP XML response - expected string, got: ' . gettype($xmlResponse));
        }
