    ...
    <div class="container-fluid px-3">
        ...
        <div class="d-flex justify-content-between align-items-center mb-3 flex-wrap gap-2">
            <h1 class="title-bar mb-0">
                <i class="fa fa-check-circle text-success"></i> {{ __('Cancelled Bookings') }}
                <span class="badge badge-success ml-2">{{ $rows->total() }} {{ __('bookings') }}</span>
            </h1>
            <div>
                <a href="{{ route('all-booking.cancelled') }}" class="btn btn-outline-secondary btn-sm">
        ...
        <div class="card shadow-sm mb-3">
            <div class="card-body py-2 px-3">
                <form method="GET" action="{{ route('all-booking.cancelled') }}" id="filter-form">
                        ...
                        <div class="col-auto">
                            ...
                            <div class="d-flex gap-1">
                                ...
                                @if(!empty($dateFrom) || !empty($dateTo) || !empty($searchTerm))
                                    <a href="{{ route('all-booking.cancelled') }}" class="btn btn-secondary btn-sm">
                    ...
                    <table id="bookings-table" class="table table-hover table-bordered table-sm mb-0" style="width:100%">
                        ...
                        @forelse($rows as $i => $booking)
                            ...
                            <tr>
                                ...
                                {{-- Status --}}
                                <td>
                                    <span class="badge badge-cancelled" style="font-size:11px">
                                        <i class="fa fa-check-circle"></i> cancelled
                                    </span>
                                    @if($hasTicket)
                            ...
                            <tr>
                                <td colspan="14" class="text-center py-4">
                                    <div class="text-muted">
                                        ...
                                        {{ __('No cancelled bookings found') }}
                                    </div>
                                </td>
                            </tr>
                        @endforelse
...
@push('css')
    ...
    <style>
        .badge-cancelled {
            background: linear-gradient(135deg, #dd1515, #ce0303);
            color: #fff;
            padding-left: 16px;
            position: relative;
        }
...
$(document).ready(function() {
    ...
    $(document).on('click', '.btn-pnr-edit', function(e) {
        ...
        $('#pnr_edit_status').val($(this).data('status') || 'cancelled');
