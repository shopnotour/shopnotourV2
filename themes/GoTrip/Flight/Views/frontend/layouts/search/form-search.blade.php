                    ...
                    <div class="flex flex-col lg:flex-row items-end gap-4 mb-4">
                        ...
                        <div class="flex-[2] flex w-full relative gap-4">
                            ...
                            <div class="flex-1 border rounded-lg px-3 py-4 relative" x-data="{ openSearch: false }">
                                ...
                                <input type="text"
                                       x-model="segment.from_name" readonly
                                ...
                                <div x-show="openSearch"
                                     @click.outside="openSearch = false; segment.tempSearch = '';"
                                     x-init="$watch('openSearch', val => val && $nextTick(() => $el.querySelector('input').focus()))"
                                     class="absolute left-0 right-0 bg-white border rounded-lg shadow-md mt-1 z-50">
                                    <input type="text"
                                           x-model="segment.tempSearch"
...
{{--                                        placeholder="Search airport..." class="border-b p-2 w-full">--}}
                                    ...
                                    <ul class="max-h-60 overflow-y-auto">
                                        <template x-for="airport in searchResults[`${index}_from`]" :key="airport.id">
                                            <li @click="selectAirport(index, 'from', airport); openSearch = false;"
                                                ...
                                                <span x-text="airport.code"></span> - ( <span x-text="airport.name"></span>)
                                            </li>
                                        </template>
                                    </ul>
                                </div>
                            ...
                            <div class="flex-1 border rounded-lg px-3 py-4 relative" x-data="{ openTo: false }">
                                ...
                                <div x-show="openTo"
                                     @click.outside="openTo = false; segment.tempTo = '';"
                                     x-init="$watch('openTo', val => val && $nextTick(() => $el.querySelector('input').focus()))"
                                     x-transition
                                     class="absolute left-0 right-0 bg-white border rounded-lg shadow-md mt-1 z-50">
                                    <input type="text"
                                           x-model="segment.tempTo"
...
{{--                            </div>--}}
                            ...
                            <div :class="tripType === 'multi' ? 'w-full' : 'w-full sm:w-1/2'"
                                 class="border rounded-lg px-3 py-4 cursor-pointer hover:bg-gray-50 transition"
                                 @click="$el.querySelector('input')._flatpickr.open()">
                                <label class="block text-sm font-medium text-red-700 mb-1 pointer-events-none">Departure Date</label>
                                <input type="text"
                                       :name="`segments[${index}][departure]`"
                                       x-model="segment.departure"
                                       x-init="flatpickr($el, {
                                            dateFormat: 'Y-m-d',
{{--                                             altInput: true,--}}
{{--                                             altFormat: 'd - m - Y',--}}
                                            minDate: 'today',
                                            defaultDate: segment.departure
                                        })"
                                       placeholder="Select date"
                                       class="border-0 p-0 w-full rounded cursor-pointer bg-transparent focus:outline-none pointer-events-none">
                            </div>

                            <!-- Return Date (Only for One Way & Round Trip) -->
                            <div x-show="tripType !== 'multi'"
                                 class="w-full sm:w-1/2 border rounded-lg px-3 py-4 transition"
                                 :class="tripType === 'oneway' ? 'bg-gray-100 cursor-not-allowed' : 'cursor-pointer hover:bg-gray-50'"
                                 @click="tripType !== 'oneway' && $el.querySelector('input')._flatpickr.open()">
                                <label class="block text-sm font-medium text-red-700 mb-1 pointer-events-none">
                                    Return Date
                                </label>
...
{{--                                             altFormat: 'd - m - Y',--}}
                                            ...
                                            clickOpens: tripType !== 'oneway'
                                        })"
                                       placeholder="Select return date"
                                       class="border-0 p-0 w-full rounded bg-transparent focus:outline-none pointer-events-none"
            ...
            <div class="mt-6 flex flex-col md:flex-row items-stretch md:items-end gap-4 border-t pt-6">
                ...
                <div class="flex md:hidden gap-4 w-full m-2">
                    ...
                    <div x-show="tripType === 'multi'" class="flex-shrink-0 w-1/2">
                        <button type="button"
                                @click="addSegment"
                                class="cursor-pointer bg-red-500 text-white font-weight-semi-bold border border-red-200 rounded-lg px-5 py-2 h-full flex flex-col justify-center  min-w-[180px]">
                            + Add Flight
                        </button>
                    </div>
{{--                    <div x-show="tripType === 'multi'" class="flex-shrink-0 w-1/2">--}}
...
{{--                    </div>--}}
                ...
                <div class="hidden md:flex md:flex-1 md:gap-4 md:justify-between md:pt-0">
                    ...
                    <div x-show="tripType === 'multi'" class="relative flex-shrink-0 h-12 mt-0  ml-4">
                        <button type="button"
                                @click="addSegment"
                                {{--                            class="bg-blue-500 hover:bg-blue-600 text-white px-6 py-5  rounded-lg font-semibold">--}}
                                class="cursor-pointer bg-red-500 text-white font-weight-semi-bold hover:bg-red-400 border border-red-200 rounded-lg px-5 py-3 h-full flex flex-col justify-center  min-w-[180px]">
                            + Add Flight
                        </button>
                    </div>
...
{{--                    </div>--}}
                ...
                <div class="flex-shrink-0 w-full md:w-auto md:ml-auto">
                    <button type="submit"
                            class="bg-yellow-400 hover:bg-yellow-500 text-gray-900 w-full lg:w-auto px-6 py-[10px]  rounded-lg font-bold text-lg shadow-md">
                        {{--                        class="cursor-pointer bg-yellow-400 hover:bg-yellow-500 text-gray-900 rounded-lg px-5 py-3 h-full flex flex-col justify-center md: min-w-[180px]">--}}
                        Search Flights
                    </button>
                </div>
{{--                <div class="flex-shrink-0 w-full md:w-auto md:ml-auto">--}}
...
{{--                </div>--}}
    ...
    function multiCityForm() {
        return {
            ...
            tripType: 'round',
            segments: [{
                from_name: 'DAC - ( Hazrat Shahjalal International Airport)',
                from_id: 61,
                to: 'CXB - ( Coxs Bazar Airport)',
                to_id: '62',
            ...
            init() {
                ...
                @if(request()->has('segments'))
                    ...
                    this.segments = [
                        ...
                        {
                            from_name: '@if(isset($segmentAirports[$index]) && isset($segmentAirports[$index]["from"]) && $segmentAirports[$index]["from"]){{ $segmentAirports[$index]["from"]->code }} - ( {{ $segmentAirports[$index]["from"]->name }})@endif',
                            from_id: '{{ $segment["from"] ?? "" }}',
                            to: '@if(isset($segmentAirports[$index]) && isset($segmentAirports[$index]["to"]) && $segmentAirports[$index]["to"]){{ $segmentAirports[$index]["to"]->code }} - ( {{ $segmentAirports[$index]["to"]->name }})@endif',
                            to_id: '{{ $segment["to"] ?? "" }}',
            ...
            selectAirport(index, field, airport) {
                const segment = this.segments[index];
                if (field === 'from') {
                    segment.from_name = `${airport.code} - ( ${airport.name})`;
                    segment.from_id = airport.id;
                    segment.tempSearch = '';
                } else {
                    segment.to = `${airport.code} - ( ${airport.name})`;
