...
export default {
    ...
    data() {
        return {
            allFlights: [], isLoading: false, sidebarFixed: false,
            activeSheet: null,
            tempMin: 0, tempMax: 200000,
    ...
    methods: {
        ...
        handleScroll() {
            const t = document.getElementById('sidebar-trigger-point');
            if (!t) return;
            this.sidebarFixed = t.getBoundingClientRect().top <= 80;
            if (this.sidebarFixed && this.$refs.desktopSidebar) {
                const col = this.$refs.desktopSidebar.closest?.('.col-lg-3');
                if (col) { const r=col.getBoundingClientRect(); this.$refs.desktopSidebar.style.left=r.left+'px'; this.$refs.desktopSidebar.style.width=r.width+'px'; }
            }
        }
    },
    mounted() {
        if (window.flightData?.flights) {
            ...
            if (prices.length) {
                this.filters.priceRange=[Math.min(...prices),Math.max(...prices)];
                this.tempMin=Math.min(...prices); this.tempMax=Math.max(...prices);
            }
        }
        window.addEventListener('scroll', this.handleScroll);
        window.addEventListener('resize', ()=>{ if(window.innerWidth>=992&&this.activeSheet) this.closeSheet(); });
    },
    beforeDestroy() { window.removeEventListener('scroll', this.handleScroll); document.body.style.overflow=''; }
};
</script>
...
/* Desktop sidebar */
.fsp-dsk-sidebar { transition: none; }
.fsp-dsk-sidebar.fixed { position: fixed; top: 100px; max-height: calc(100vh - 120px); overflow-y: auto; z-index: 50; scrollbar-width: thin; scrollbar-color: #cbd5e1 #f1f5f9; }
.fsp-dsk-sidebar.fixed::-webkit-scrollbar { width: 4px; }
.fsp-dsk-sidebar.fixed::-webkit-scrollbar-thumb { background: #cbd5e1; border-radius: 4px; }
