...
const AirportList = {
    ...
    template: `
        <div class="fsf-airport-list">
            ...
            <template v-else-if="showSearch">
                <template v-if="results && results.length">
                    <div v-for="a in results" :key="a.id"
                        ...
                        <div class="fsf-al-code">{{ a.code }}</div>
                        <div class="fsf-al-name">{{ a.name }}</div>
                        <i class="fa fa-plane fsf-al-icon"></i>
                    </div>
                </template>
            ...
            <template v-else>
                <div v-if="recents.length" class="fsf-al-section">Recent</div>
                <div v-for="a in recents" :key="'r-' + a.id"
                     class="fsf-al-item" @mousedown.prevent="pick(a)">
                    <div class="fsf-al-code">{{ a.code }}</div>
                    <div class="fsf-al-name">{{ a.name }}</div>
                    <i class="fa fa-clock fsf-al-icon"></i>
                </div>
...
export default {
    ...
    methods: {
        ...
        handleSearch() {
            ...
            // Advanced params
            if (this.advStops)          p.append('stops', this.advStops);
            if (this.advAirline)        p.append('airline', this.advAirline);
            if (this.advDepartureTime)  p.append('departure_time', this.advDepartureTime);
            if (this.advMaxPrice)       p.append('max_price', this.advMaxPrice);
            if (this.advHandBaggage)    p.append('hand_baggage', 1);
            if (this.advCheckedBaggage) p.append('checked_baggage', 1);
            if (this.advRefundable)     p.append('refundable', 1);
            if (this.advNonStop)        p.append('non_stop', 1);

            window.location.href = `${window.flightData?.searchUrl || '/flight/search'}?${p.toString()}`;
        }
    },

    mounted() {
        const hasSession = window.flightData?.searchParams && Object.keys(window.flightData.searchParams).length > 0;
...
.fsf-al-code { font-weight: 800; font-size: 15px; color: var(--fsf-primary); min-width: 42px; }
.fsf-al-name { flex: 1; font-size: 12px; color: var(--fsf-muted); white-space: nowrap; overflow: hidden; text-overflow: ellipsis; }
.fsf-al-icon { font-size: 11px; color: #cbd5e1; }
...
@media (max-width: 600px) {
    ...
    .fsf-tab span { display: none; }  /* icon only on very small */
